...
export const normalizeBackendResult = (
  ...
  revisionNumber: item.revisionNumber ?? index + 1,
  revisionLength: item.revisionLength ?? 0,
  coversWholeDocument: Boolean(item.coversWholeDocument),
  laterRevisionsPresent: Boolean(item.laterRevisionsPresent),
